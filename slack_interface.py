...
Examples:
  ...
  %(prog)s info "#general"           Get channel info
    ...
    if not tokens.access_token and not tokens.bot_token:
        print("❌ No Slack tokens found", file=sys.stderr)
        print(f"   Checked file: {args.token_file}", file=sys.stderr)
        print(f"   Checked env vars: SLACK_TOKEN, SLACK_BOT_TOKEN, SLACK_MCP_XOXP_TOKEN, SLACK_MCP_XOXB_TOKEN", file=sys.stderr)
        print(f"\n💡 To set tokens, either:", file=sys.stderr)
        print(f"   1. Add Slack tokens to {args.token_file}", file=sys.stderr)
        print(f"   2. Set environment variables:", file=sys.stderr)
        print(f"      export SLACK_TOKEN='xoxp-...'", file=sys.stderr)
        print(f"      export SLACK_BOT_TOKEN='xoxb-...'", file=sys.stderr)
        return
